...
set.seed(12345)
######### NETWORKS
folder_path <- "C:/Users/rodri/OneDrive/Documentos/DOCTORADO/Paper Flins Inske Datos/Redes/Tabla Modularidades/Nuevo Real Case"
...
precompute_matrices <- function(graph_list) {
  ...
  for (file in graph_list) {
    ...
    g <- read_graph(file_list, format = "graphml")
...
graph_name <- file_list[1]
weight_combinations <- weight_combinations
algorithm <- 'louvain'
matrices_list <- precomputed_matrices
...
for (file in network_list) {
  ...
  final_results <- rbind(final_results, louvain)
  final_results$modularity <- ifelse(is.nan(final_results$modularity), 0, final_results$modularity)
  
  write.csv(final_results, paste0("C:/Users/rodri/OneDrive/Documentos/DOCTORADO/Paper Flins Inske Datos/Redes/Tabla Modularidades/Nuevo Real Case/", substr(network_name, 1, nchar(network_name) - 8), "_modularity_results.csv"), row.names = FALSE)
}
